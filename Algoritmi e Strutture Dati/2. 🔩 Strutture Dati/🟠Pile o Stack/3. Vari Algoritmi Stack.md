...
>
>- $T_{migliore}(n) \Rightarrow Ω(min(n, m)) \rightarrow$ l'elemento in cima allo stack con più elementi è maggiore di tutti gli elementi dell'altro stack, così da scorrere un numero di elementi pari al minimo tra $n$ e $m$
>- $T_{peggiore}(n) \Rightarrow O(n+m) \rightarrow$ l'elemento in fondo allo stack $P$ è maggiore di tutti gli elementi di $Q$ escluso quello in fondo a $Q$, e viceversa, l'elemento in fondo allo stack $Q$ è maggiore di tutti gli elementi di $P$ escluso quello in fondo a $P$ così da scorrere tutti gli elementi di $P$ + quelli di $Q$ nel primo ciclo $while$