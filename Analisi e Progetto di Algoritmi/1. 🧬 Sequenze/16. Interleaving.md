...
- $wi+j ̸= xi ∧ wi+j ̸= yj$ : in questo caso la soluzione `e molto semplice, se non possono essere accodati n´e xi n´e yj al simbolo in posizione wi+j allora Wi+j non pu`o essere interleaving di Xi e Yj . In questo caso quindi si,j = F alse. In questo caso quindi siamo di fronte ad un caso base. Riassumendo, possiamo scrivere questo caso come:
