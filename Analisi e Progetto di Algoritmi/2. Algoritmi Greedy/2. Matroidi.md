...
>
>**Si può dimostrare** che un Sistema di Indipendenza $(S,F)$ è un **matroide** se e solo se per ogni $C ⊆ S$ gli insiemi massimali di $(C, F_C)$ hanno la *stessa cardinalità*.
>
>L’*algoritmo greedy standard* fornisce sempre (per costruzione) **insiemi massimali**.

Supponiamo che $(S, F)$ sia un **matroide**, e sia $W$ la funzione peso e $X = \{s_1, s_2, …, s_p\}$ la soluzione fornita dall’*algoritmo greedy standard*.
Voglio dimostrare che per qualsiasi $X'$ **massimale**, si ha $W(X) ≥ W(X')$.

Per costruzione, $X$ è massimale e $W(s_1) ≥ W(s_2) ≥ … ≥ W(s_p)$.
Sia $X' = \{s'_1, s'_2, …, s'_p\}$ un insieme massimale **diverso** da $X$ (stessa cardinalità di $X$) tale che $W(s'_1) ≥ W(s'_2) ≥ … ≥ W(s'_p)$.

- **CASO 1**: $W(s_i) ≥ W(s'_i)$, $∀ 1 ≤ i ≤ p \Longrightarrow W(X) ≥ W(X')$
- **CASO 2**: $W(s_i) < W(s'_i)$ per qualche $i$
Sia $k$ il più piccolo indice tale che $W(s_k) < W(s'_k)$
$$\Longrightarrow W(s_k) < W(s'_k) ≤ W(s'_{k-1}) ≤ W(s_{k-1})$$
...
$\Longrightarrow |X' ∩ C| > |X ∩ C|$ in contraddizione con il fatto che $X$ e $X' \text{ sono massimali per } (S,F)$
