...
2. Supponiamo ora che $(S, F)$ **non** sia un matroide
	Esiste un insieme $C ⊆ S$, ed esistono due insiemi $A$ e $B$ massimali in $(C, F_C)$ con $|B| = p$ e $|A| > |B|$.
	Si consideri il seguente peso $W$:
	- $W(s) = p+2$ se $s ∈ B$
	- $W(s) = p+1$ se $s ∈ A – B$
	- $W(s) = 1$ altrimenti
	L’algoritmo greedy restituisce $B$.
	$W(A) ≥ (p+1)(p+1) = p2 + 2p + 1 > p2 + 2p = p(p+2) = W(B)$