...
*Dato un grafo $G = (V, E, W, col)$ e $k ∈ {0, 1, . . . , n}$, si vuole calcolare per ogni coppia $(i, j) ∈ V × V$ il peso di un cammino minimo $p$ da $i$ a $j$, con vertici intermedi appartenenti a $\{1, 2, . . . , k\}$.* 
...
**Soluzione del problema**: $D^{(n)}$, dove $D$ è la **matrice** dei cammini minimi per ogni coppia $(i, j) ∈ V × V$
