...
>[!Corollario]
>Sia $A ⊆ T$ è tale che $G_A = (V,A)$ è una foresta di $|V|-|A|$ alberi.
>Sia $C = (V_C, A_C)$ tale che $V_C ⊆ V$ e $A_C ⊆ A$, una componente connessa di $G_A$, allora $(V_C, V- V_C)$ è sicuramente un taglio che rispetta $A$ dove un arco leggero $(u, v)$ di $(V_C, V- V_C)$ è arco sicuro per $A$.
