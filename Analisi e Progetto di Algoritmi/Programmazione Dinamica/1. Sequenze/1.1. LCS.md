...
1. Se $x_m = y_n$ allora $z_k = x_m = y_n$ e $Z_{k-1} = LCS(X_{m-1}, Y_{n-1})$ $$LCS(X, Y) = LCS(X_{m-1}, Y_{n-1}) + ⟨x_m⟩$$
2. Se $x_m \neq y_n$ e:
	- $z_k ≠ x_m$ allora $Z = LCS(X_{m-1}, Y_n)$
	- $z_k ≠ y_n$ allora $Z = LCS(X_{m}, Y_{n - 1})$
$$LCS(X, Y) = max\{LCS(X_{m-1}, Y_{n}), LCS(X_{m}, Y_{n-1})\}$$
