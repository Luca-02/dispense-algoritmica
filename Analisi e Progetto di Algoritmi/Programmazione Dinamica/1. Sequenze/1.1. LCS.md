...
#### Dimostrazione
1. Se $z_k\not=x_m$ allora potremmo accodare $x_m=y_n$ a $Z$ per ottenere una sottosequenza comune di $X$ e $Y$ di lunghezza $k+1$, contraddicendo l’ipotesi che $Z$ è la più lunga sottosequenza comune di $X$ e $Y$. Quindi, deve essere $z_k=x_m=y_n$. Ora, il prefisso $Z_{k−1}$ è una sottosequenza comune di $X_{m−1}$ e $Y_{n−1}$ di lunghezza $k−1$, vogliamo dimostrare che questo prefisso è una $LCS$.
