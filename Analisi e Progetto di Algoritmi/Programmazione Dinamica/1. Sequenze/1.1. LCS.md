...
Date $X=⟨x_1, x_2, …, x_{m-1}, x_m⟩$ e $Y=⟨y_1, y_2, …, y_{n-1}, y_n⟩$ due sequenze e sia $Z = ⟨z_1, z_2, …, z_{n-1}, z_n⟩$ una qualunque $LCS$ di $X$ e $Y$:

- $x_m = y_n \Rightarrow LCS(X, Y) = LCS(X_{m-1}, Y_{n-1}) + ⟨x_m⟩$
- $x_m \neq y_n \Rightarrow LCS(X, Y) = max(LCS(X_{m-1}, Y_{n}), LCS(X_{m}, Y_{n-1}))$ 
...
- Il caso base si ha per un qualunque sottoproblema di dimensione $(i, j)$ con $i = 0 \lor j = 0$, ossia quando uno dei due prefissi considerati è la `sequenza vuota`.
	...
	$$LCS(X_i , Y_j) = ε \quad\text{se } i = 0 \lor j = 0$$
...
1. $x_i = y_j$ : 
	...
	$$LCS(X_i, Y_j) = LCS(X_{i-1}, Y_{j-1}) + ⟨x_i⟩ \quad\text{se } x_i = y_j$$
...
2. $x_i \neq y_j$ :
	...
	$$LCS(X_i, Y_j) = max\{LCS(X_{i-1}, Y_{j}), LCS(X_{i}, Y_{j-1})\} \quad\text{se } x_i \neq y_j$$
