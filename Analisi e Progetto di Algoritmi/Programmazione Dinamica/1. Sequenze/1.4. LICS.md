...
Siano $X=⟨x_1, x_2, …, x_{m-1}, x_m⟩$ e $Y=⟨y_1, y_2, …, y_{n-1}, y_n⟩$ due sequenze di numeri interi e siano $X_i$ e $Y_j$ due loro prefissi. Sia $Z^{ij}$ una $LICS$ di $X_i$ e $Y_j$ e che termina con $x_i=y_j$. Allora vale che $$Z^i = Z^∗ |x_i$$con $Z^∗ ∈ W_{ij}$ e $|Z^∗ | = max_{W∈W_{ij}} \{|W|\}$ dove $W_{ij}$ è l’insieme di tutte le sottosequenze crescenti comuni a $X_h$ e $Y_k$ a cui è possibile concatenare $x_i=y_j$.
#### Dimostrazine
**Per assurdo**
...
Inoltre, siano $h < i$ e $k < j$ i più grandi indici tale che $x_h = z^′$ . Di conseguenza si ottiene che $Z^′ ∈ W_i$. Infatti $Z^′$ è una sottosequenza crescente di $X_h$, la quale termina con $x_h < x_i$. 
